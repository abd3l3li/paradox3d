...
/*   Updated: 2025/03/07 01:00:00 by her-rehy         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_collision(t_cube *cube, t_distance *d, t_dis *dis, float *hit_x,
		float *hit_y)
{
...
float	cast_ray(t_cube *cube, t_distance *d, t_dis *dis, float *hit_x,
		float *hit_y)
{
	while (1)
	{
		...
		if (dis->my < 0 || dis->my >= dis->map_height || dis->mx < 0
			...
			break ;
		if (check_collision(cube, d, dis, hit_x, hit_y))
...
float	distance_to_wall(t_cube *cube, t_distance *d, float angle, float *hit_x,
		float *hit_y)
{
	static t_dis	dis;
	static int		row;
...
void	cube_render(t_cube *cube)
{
	...
	{
		raw_distance = distance_to_wall(cube, &d, render.ray_angle, &hit_x,
				&hit_y);
		...
		draw_vertical_line(cube, &d, ray, wall_height, render.color, hit_x,
			hit_y);
